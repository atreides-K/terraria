...
// MData
struct alignas(16) MData {
    std::array<float, 2> offset;
    float scale;
    uint32_t level;
};
wgpu::Buffer mDataBuffer;
...
void Render() {
  ...
  MData mdata{};
  mdata.offset = { 2.0f, 2.0f };
  mdata.scale  = 1.0f;
  mdata.level  = 0;


  device.GetQueue().WriteBuffer(mDataBuffer, 0, &mdata, sizeof(MData));
  ...
  pass.SetBindGroup(1, mDataBindGroup);
...
void InitGraphics() {
  ...
  std::vector<wgpu::BindGroupLayout> bindGroupLayouts = {cameraBindGroupLayout, mDataGroupLayout};
  ...
  mDataBuffer = BufferUtils::createUniformBuffer(device, sizeof(MData));
